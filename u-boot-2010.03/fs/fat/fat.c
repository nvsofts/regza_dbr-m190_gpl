	...
	while (1) {
		...
		if ((mydata->fatsize == 32) && (++j == mydata->clust_size)) {
			...
			if (CHECK_CLUST(nxt_clust, mydata->fatsize))
				/* if nxt_clust == 0x0fffffff (last entry of chain) */
				return -1;
			...
			cursect++;
			if (((mydata->fatsize == 12) || (mydata->fatsize == 16)) &&
			    (cursect == (mydata->rootdir_sect + rootdir_size)))
				/* on FAT12/16, end of root entries */
				return -1;
		}
	}
rootdir_done:
